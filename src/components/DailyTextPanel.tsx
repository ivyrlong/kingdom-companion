...
function Youth({
  ...
  // Deterministic order on first render so SSR HTML matches the client's
  // hydration pass; then shuffle once after mount (client-only).
  const [bank, setBank] = useState<string[]>(answers);
  useEffect(() => {
    setBank(shuffle(answers));
    // Only re-shuffle when the underlying answer set changes.
